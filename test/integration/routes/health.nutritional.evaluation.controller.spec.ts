...
describe('Routes: HealthProfessionalsNutritionalEvaluation', () => {
    before(async () => {
            try {
                ...
                throw new Error('Failure on HealthProfessionalsNutritionalEvaluation test: ' + err.message)
            }
        }
    )
    ...
    describe('GET /v1/healthprofessionals/:healthprofessional_id/nutritional/evaluations', () => {
        context('when want get a list of nutrition evaluations from health professional', () => {
            it('should return status code 200 and a list of evaluations', () => {
                const url: string = `/v1/healthprofessionals/${DefaultEntityMock.NUTRITION_EVALUATION.health_professional_id}/` +
        ...
        context('when there no are evaluations', () => {
            it('should return status code 200 and a empty list', () => {
                return request
                    .get(`/v1/healthprofessionals/${new ObjectId()}/nutritional/evaluations`)
                    .set('Content-Type', 'application/json')
                    .expect(200)
                    .then(res => {
                        expect(res.body).to.be.an.instanceof(Array)
        ...
        context('when there are validation errors', () => {
            it('should return status code 400 and message from invalid parameters', () => {
                return request
                    .get('/v1/healthprofessionals/123/nutritional/evaluations')
                    .set('Content-Type', 'application/json')
                    .expect(400)
                    .catch(err => {
